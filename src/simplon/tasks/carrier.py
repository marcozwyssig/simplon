...
def _pulumi(carrier: carrierspec.Carrier, token: str, program: Path, state: Path, hostname: str) -> int:
    ...
    argv = [
        ...
        "-e", f"PULUMI_CONFIG_PASSPHRASE={os.environ.get(PASSPHRASE_ENV, '')}",
        "-e", "PULUMI_BACKEND_URL=file:///state",
        "-e", f"CARRIER_ENDPOINT={carrier.proxmox.endpoint}",
        "-e", f"CARRIER_TOKEN={token}",
...
def _ansible(carrier: carrierspec.Carrier, address: str, work: Path, key: Path, port: int,
             password: str) -> int:
    ...
    (work / "carrier.yml").write_text(
        PLAYBOOK.format(image=PORTAINER_IMAGE, port=port, password=password), encoding="utf-8")
...
def up(environment: str = "") -> int:
    ...
    password = portainer_password(carrier)
    ...
    if not address:
        ...
        return 1
    return _ansible(carrier, address, program, key_path, PORTAINER_PORT, password)
