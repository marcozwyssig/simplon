...
- name: A Proxmox container becomes a Portainer carrier
  ...
  tasks:
    ...
    - name: Is Portainer already there?
      ansible.builtin.command:
    ...
    - name: What it reports
      ansible.builtin.debug:
        msg: "Portainer {{{{ status.json.Version }}}} on {{{{ ansible_host }}}}:{{{{ portainer_port }}}}"
'''
...
def _ansible(carrier: carrierspec.Carrier, address: str, work: Path, key: Path, port: int) -> int:
    ...
    (work / "inventory.ini").write_text(
        f"[carrier]\ncarrier ansible_host={address} ansible_user=root\n", encoding="utf-8")
    argv = [
        "docker", "run", "--rm",
