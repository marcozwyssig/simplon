...
The MECHANISM - probe, reconcile, report - is `simplon.nexusproxy` and is covered by test_nexus.py. What is
...
def test_nexus_cmd_returns_the_dispatch_exit_code_rather_than_raising_typer_exit(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr("simplon.nexusproxy.dispatch", lambda member: seen.append(member) or 3)
...
def test_nexus_cmd_passes_a_missing_member_through_as_none(monkeypatch):
    ...
    seen = []
    monkeypatch.setattr("simplon.nexusproxy.dispatch", lambda member: seen.append(member) or 0)
